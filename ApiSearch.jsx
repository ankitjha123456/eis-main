...
function ApiSearch() {
  ...
  // ─── ✅ FIX: Refresh only a single API after start/stop ───────────────────────
  // Hits the search endpoint, finds fresh rows for this apiName, patches them in state.
  const refreshSingleApi = async (apiName) => {
    try {
      const url = endpoints[activeEnv].search;
      const response = await fetch(url);
      ...
      let jsonData;
      try {
        const fixedText = responseText.startsWith("[") ? responseText : `[${responseText}]`;
      ...
      // Only patch rows matching this apiName
      const freshEntries = jsonData.filter((d) => d.ApiName === apiName);
      if (freshEntries.length === 0) return;

      setData((prev) =>
        prev.map((existing) => {
          const match = freshEntries.find(
            (f) =>
              f.ApiName === existing.ApiName &&
              f.IntegrationServer === existing.IntegrationServer
          );
          if (!match) return existing;
          return {
            ...existing,
            ApiState: match.ApiState || existing.ApiState,
            DeployedDate: match.DeployedDate || existing.DeployedDate,
          };
        })
      );
    } catch (err) {
      console.warn("Single API refresh failed:", err);
    }
  };
  ...
  // ─── ✅ FIX: Start / Stop — updates ALL sibling servers, then refreshes only that API ──
  const handleServiceAction = async (action, item, username, password) => {
    ...
    try {
      ...
      // ✅ FIX 1: Update state for ALL entries with same ApiName + IntegrationNode
      // This covers sibling servers (e.g. IP .50 and .51) that share the same node.
      const newState = action === "start" ? "running" : "stopped";
      setData((prev) =>
        prev.map((d) =>
          d.ApiName === item.ApiName && d.IntegrationNode === item.IntegrationNode
            ? { ...d, ApiState: newState }
            : d
        )
      );
      ...
      // ✅ FIX 2: Auto-refresh only this API from server to get confirmed state
      await refreshSingleApi(item.ApiName);
    } catch (err) {
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      const matchesIntegrationNode =
        filters.integrationNode === "" ||
        (item.IntegrationNode &&
          item.IntegrationNode.toLowerCase().includes(filters.integrationNode.toLowerCase()));
      const matchesIntegrationServer =
        filters.integrationServer === "" ||
        (item.IntegrationServer &&
          item.IntegrationServer.toLowerCase().includes(filters.integrationServer.toLowerCase()));
      const matchesServerIP =
        filters.serverIP === "" || (item.ServerIP && item.ServerIP.includes(filters.serverIP));
      const matchesState =
        filters.state === "all" ||
      ...
      return (
        matchesApiName &&
        matchesIntegrationNode &&
        matchesIntegrationServer &&
        matchesServerIP &&
        matchesState
      );
    });
  ...
  const uniqueValues = useMemo(() => {
    ...
    if (filters.serverIP)
      filteredDataForValues = filteredDataForValues.filter((item) => item.ServerIP === filters.serverIP);
    if (filters.serverIP && filters.integrationNode)
      filteredDataForValues = filteredDataForValues.filter(
        (item) => item.IntegrationNode === filters.integrationNode
      );
  ...
  const groupedData = useMemo(() => {
    ...
    if (filters.serverIP)
      dataToGroup = dataToGroup.filter((item) => item.ServerIP === filters.serverIP);
  ...
  const toggleAllServers = () => {
    setExpandedServers(
      expandedServers.size === Object.keys(groupedData).length
        ? new Set()
  ...
  const isDownloading = (apiName, integrationServer) =>
    downloadingApis.has(`${apiName}-${integrationServer}`);
  const getServiceAction = (apiName, integrationServer) =>
    serviceActionMap[`${apiName}-${integrationServer}`] || null;

  // ─── Total counts ─────────────────────────────────────────────────────────────
  const totalApis = filteredData.length;
  const runningApis = filteredData.filter((d) => isApiRunning(d.ApiState)).length;
  const stoppedApis = totalApis - runningApis;
      ...
      {modal.open && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Modal Header */}
            <div
              className={`modal-header1 ${
                modal.action === "start" ? "modal-header-start" : "modal-header-stop"
              }`}
            >
            ...
            <div className="modal-body">
              <p className="modal-info">
                Enter credentials to {modal.action === "start" ? "start" : "stop"} this API on{" "}
                <strong>{modal.item?.IntegrationServer}</strong>
              </p>
              ...
              <div className="modal-field">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setModal((prev) => ({ ...prev, username: e.target.value, error: "" }))
                  }
              ...
              <div className="modal-field">
                ...
                <div className="modal-password-wrapper">
                  ...
                  <button
                    ...
                    onClick={() =>
                      setModal((prev) => ({ ...prev, showPassword: !prev.showPassword }))
                    }
                    type="button"
                  >
                    {modal.showPassword ? "🙈" : "👁"}
            ...
            <div className="modal-footer">
              ...
              <button
                className={`modal-confirm-btn ${
                  modal.action === "start" ? "confirm-start" : "confirm-stop"
                }`}
        ...
        <>
          ...
          <div className="summary-cards" style={{ marginBottom: "24px" }}>
            <div className="summary-card">
              <h3>Total APIs</h3>
              <div className="count">{totalApis}</div>
              <div className="label">across all servers</div>
            </div>
            <div className="summary-card">
              <h3>Running</h3>
              <div className="count" style={{ color: "#22c55e" }}>{runningApis}</div>
              <div className="label">currently active</div>
            </div>
            <div className="summary-card">
              <h3>Stopped</h3>
              <div className="count" style={{ color: "#ef4444" }}>{stoppedApis}</div>
              <div className="label">currently inactive</div>
            </div>
            <div className="summary-card">
              <h3>Servers</h3>
              <div className="count">{Object.keys(groupedData).length}</div>
              <div className="label">integration servers</div>
            </div>
          </div>
          ...
          <div className="filters-section">
            ...
            <div className="filters-grid hierarchical-filters">
              ...
              <div className="filter-group">
                ...
                >
                  ...
                  {uniqueValues.integrationServers.map((srv) => (
            ...
            <div className="filter-stats">
              <span className="results-count">
                Showing <strong>{filteredData.length}</strong> of <strong>{data.length}</strong> APIs
              </span>
            </div>
          </div>
          ...
          {Object.keys(groupedData).length === 0 ? (
            <div className="empty-state">
              ...
              <button className="secondary-button" style={{ marginTop: "16px" }} onClick={clearFilters}>
                Clear Filters
              </button>
            </div>
          ) : (
            <div className="servers-section">
              ...
              <div className="servers-grid">
                {Object.entries(groupedData).map(([serverName, serverData]) => {
                  ...
                  const allRunning = serverData.runningCount === serverData.totalCount;
                  const noneRunning = serverData.runningCount === 0;
                      ...
                      >
                        <div className="server-info">
                          ...
                          <div className="server-meta">
                            <span className="server-ip">🌐 {serverData.serverIP}</span>
                            <span className="server-node">⚙ {serverData.integrationNode}</span>
                          </div>
                        </div>
                        <div className="server-right">
                          <div className="server-stats">
                            <span className="total">📦 {serverData.totalCount} APIs</span>
                            <span className={`running ${allRunning ? "all-running" : ""}`}>
                              🟢 {serverData.runningCount} running
                            </span>
                            {serverData.totalCount - serverData.runningCount > 0 && (
                              <span className="stopped">
                                🔴 {serverData.totalCount - serverData.runningCount} stopped
                              </span>
                            )}
                          </div>
                          ...
                          <table className="server-table">
                            ...
                            <tbody>
                              {serverData.items.map((item, index) => {
                                ...
                                const actionInProgress = getServiceAction(
                                  item.ApiName,
                                  item.IntegrationServer
                                );
                                ...
                                return (
                                  <tr key={`${item.ApiName}-${item.IntegrationServer}-${index}`}>
                                    <td>
                                      <span className="api-name-text">{item.ApiName}</span>
                                    </td>
                                    <td>
                                      <span className="eg-tag">{item.IntegrationNode}</span>
                                    </td>
                                    <td>
                                      <span className="ip-address">{item.ServerIP}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                      <div className="action-cell">
                                        {/* Start Button — show only when stopped */}
                                        {!running && (
                                          <button
                                            className={`service-btn start-btn ${
                                              actionInProgress ? "in-progress" : ""
                                            }`}
                                            onClick={() => openModal("start", item)}
                                        ...
                                        {/* Stop Button — show only when running */}
                                        {running && (
                                          <button
                                            className={`service-btn stop-btn ${
                                              actionInProgress ? "in-progress" : ""
                                            }`}
                                            onClick={() => openModal("stop", item)}
                                            disabled={!!actionInProgress}
                                          >
